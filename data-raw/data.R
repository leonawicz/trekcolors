...
usethis::use_data(trekpals)

# library(imgpalr)
# f <- function(file){
#   pals <- purrr::map(c("qual", "seq", "div"), ~image_pal(file, 5, .x, c(0.01, 0.99), c(0.01, 1), c(0.01, 1), "hsv", 1))
#   names(pals) <- basename(file)
#   pals
# }
# files <- list.files("../treklogos/png", full.names = TRUE, recursive = TRUE)
# logopals <- unlist(purrr::map(files, f))
